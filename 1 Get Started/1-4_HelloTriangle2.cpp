...
int main(void) {
	...
	unsigned int VBO, VAO,	EBO;//这是三个引用ID
	...
	*/
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);//告诉OpenGL该如何解析顶点数据，
																				 ...
																				 第二个：这里设置为3，该参数指定属性(数组)的大小，或者说长度：
	...
	//下一步操作请注意！！
	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);//！！！这里不能这么做
												/*
													文中有给出相关解释：
													当目标是GL_ELEMENT_ARRAY_BUFFER的时候，VAO保存glBindBuffer的调用，
													因此解绑的操作也会被VAO所识别
													（实际上可以理解为该操作并非必须，所以允许解除或绑定，但是从VBO获取数据是不可避免的，所以VBO的解绑操作）
													但是在解绑VAO之后，解绑EBO是没有问题的！因为在绑定VAO的时候，会根据VAO自身信息进行EBO的绑定
												*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//使用EBO的时候，这样绘制（而不是使用glDrawArray()）：
		//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
		
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);//	glDrawElements函数从当前绑定到GL_ELEMENT_ARRAY_BUFFER目标的EBO获取索引。	
															/*	第一个参数：模式
																第二个参数：想要绘制的元素的数量
																第三个参数：索引的数据类型
																第四个参数：指定EBO的偏移量，
															*/
		//glBindVertexArray(0);//绘制完成之后进行解绑
